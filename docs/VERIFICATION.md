...
npm run build
npm run check
npx hyperframes@0.8.22 snapshot . --at 0.4,2.2,4.8,7.7 --output ./snapshots
npx hyperframes@0.8.22 render . --quality high --workers 2 --output ../outputs/parallel-benchmark-capture.mp4
```
...
single, ordered, version-1 ParallelHue sidecar reconciles every stream.
